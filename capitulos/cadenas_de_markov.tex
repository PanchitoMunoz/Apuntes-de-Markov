...
\begin{lem}\label{lem:aperiod}\MarginNote{$i$ aperiódico $\implies$ $X$ lo sea}
...
En vista de la observación anterior, no tenemos por qué esperar que\MarginNote{No toda CM tiene medida invariante asociada} \hly{toda cadena de Markov tenga una medida invariante asociada}, ni mucho menos una distribución invariante.
...
El principal resultado de esta sección mostrará que \hly{la existencia de distribuciones invariantes} \hlo{est{\'a} {\'i}ntimamente relacionada con una noci{\'o}n m{\'a}s fuerte de recurrencia}, que definimos a continuación:

\begin{defn}\tbf{Recurrencia positiva}\MarginNote{Recurrencia positiva}
Sea $i$ un estado recurrente.
Decimos que \hlb{$i$ es \emph{recurrente positivo}} si \hlg{su tiempo de retorno esperado es finito}, es decir, si $\ee_i(\tau_i)<\infty$.
En caso contrario, decimos que $i$ es \emph{recurrente nulo}.
\end{defn}

\begin{thm}\tbf{Existencia de distribuciones invariantes}\label{thm:ex-inv}\MarginNote{Existencia de distribuciones invariantes}
Sea \hlg{$X$ irreducible}.
Entonces \hlo{las siguientes son equivalentes}:
\begin{enumerate}[label=\uptext{(\roman*)}]
...
\begin{thm}\label{thm:uniq-inv}\MarginNote{Dist. invariante es única}
Sea \hlg{$X$ irreducible}.
Si \hlg{$X$ tiene una distribuci{\'o}n invariante $\pi$}, entonces \hlo{toda medida invariante es un m{\'u}ltiplo de $\pi$}.
En particular, \hlo{la distribuci{\'o}n invariante es {\'u}nica}.
\end{thm}

De estos dos teoremas se sigue directamente el siguiente:

\begin{cor}\label{cor:irrednorecpos}\MarginNote{Dist. invariante no existe}
Si \hlg{$X$ es irreducible y tiene una medida invariante con masa infinita}, entonces \hlo{no existe distribuci{\'o}n invariante} (y en particular la cadena no es recurrente positiva).
\end{cor}

De esta forma tenemos, para \hlg{$X$ irreducible}:
\begin{enumerate}[label=\arabic*.]
\item La \hlo{existencia de una distribuci{\'o}n invariante} equivale a que \hlo{la cadena sea recurrente positiva}.
\item Si \hlg{existe una medida invariante infinita}, entonces \hlo{la cadena es recurrente nula o transiente}.
\item Se puede demostrar también (\ucmark) que si \hlg{$X$ es recurrente nula} entonces \hlo{existe una medida invariante infinita}.
\end{enumerate}
...
La cantidad $\mathcal{E}(\mu)$ que introdujimos en la demostración anterior se conoce usualmente como la \emph{entropía relativa} o la\MarginNote{Divergencia de Kullback-Leibler} \emph{divergencia de Kullback-Leibler} de $\mu$ con respecto a $\pi$ (típicamente con la elección de $\varphi(x)=-\log(x)$, aunque en este caso levantamos el requerimiento de que $\varphi$ sea acotada, y extendemos la definición a $0$ mediante considerar $\lim_{x\to0^+}x\log(x)=0$).
...
\noindent La idea es que\MarginNote{$\mathcal{E}(\mu)$ mide la distancia} \hlo{$\mathcal{E}(\mu)$ mide la distancia}, o más precisamente \hlo{el desorden}, de $\mu$ con respecto a $\pi$ (note por ejemplo que $\mathcal{E}(\pi)=0$ para el caso $\varphi(x)=-\log(x)$).
Una consecuencia de la demostración anterior es la siguiente: la entropía relativa de cualquier distribución $\mu$ (no necesariamente invariante) con respecto a $\pi$ \hlo{decrece luego de aplicar $P$}.
Esto sugiere que \hlo{las iteraciones sucesivas de una cadena de Markov acercan su distribuci{\'o}n a la distribuci{\'o}n de equilibrio $\pi$}; esto será el tema de la siguiente sección.
\end{rem}
...
\begin{prop}\label{prop:timeconv}\MarginNote{Interpretación de la Ec.~\eqref{eq:inv-form}}
Sea \hlg{$P$ irreducible y recurrente}.
...
el\MarginNote{Definición de $\gamma_{i}^{k}$} \hlb{n{\'u}mero de visitas a $i$ antes de retornar a $k$}.

\begin{prop}\label{prop:gammaik}\MarginNote{Propiedades de $\gamma_i^k$}
Sea \hlg{$P$ irreducible y recurrent}.
...
\end{enumerate}
Más aún, $\mu=\gamma^k$ \hlo{es la {\'u}nica medida invariante tal que $\mu_k=1$}.
\end{prop}
...
\begin{cor}\label{cor:recnul}\MarginNote{Existencia de medida invariante infinita}
Si \hlg{$X$ es irreducible y recurrente nula} entonces \hlo{existe una medida invariante infinita}.
\end{cor}
...
\begin{thm}\label{thm:ergodMC}\tbf{Teorema Ergódico para Cadenas de Markov}\MarginNote{Teo. Ergódico para CM}
Sea \hlg{$P$ irreducible con distribuci{\'o}n invariante $\pi$}.
Sea $f\!:I\longrightarrow\rr$ tal que $\sum_i|f(i)|\pi(i)<\infty$ (es decir $f\in L^1(I,\pi)$).
\hlo{Entonces} para cualquier condición inicial sobre la cadena de Markov $X$ se tiene
\[\lim_{n\to\infty}\frac1{n}\sum_{k=0}^{n-1}f(X_k)=\sum_if(i)\pi(i)\]
casi seguramente.
\end{thm}

Este tipo de resultados se conocen también como \hlo{leyes de grandes n{\'u}meros para funcionales aditivos} (en este caso, de una cadena de Markov recurrente positiva).
...
Hacemos una pausa ahora en nuestro desarrollo de la teoría de cadenas de Markov para \hly{estudiar en mayor detalle las propiedades de recurrencia de marchas aleatorias en $\zz^d$}.
...
\begin{multline*}
    p^{(2n)}_{00}\leq\binom{2n}{n}\left(\frac1{12}\right)^{n}\binom{n}{m\,m\,m}\sum_{\substack{i,j,k\geq0\\\uptext{t.q. }i+j+k=n}}\binom{n}{i\,j\,k}\left(\frac13\right)^{n}\\
    =\binom{2n}{n}\left(\frac1{12}\right)^{n}\binom{n}{m\,m\,m}\sim\frac{C}{n^{3/2}}
\end{multline*}
}
...
\begin{thm}\label{thm:recSRW}\MarginNote{Teo. sobre el paseo aleatorio multidimensional}
Sea \hlg{$X$ el paseo aleatorio sim{\'e}trico en $\zz^d$} (dado por $X_n=\xi_1+\dotsm+\xi_n$ con $\big(\xi_i\big)_{i\geq1}$ i.i.d. con $\xi_1$ elegido uniformemente entre $\{e_1,-e_1,\dotsc,e_d,-e_d\}$, con $e_i$ el $i$-ésimo vector canónico en $\zz^d$).
Entonces \hlo{$X$ es recurrente nula si $d=1,2$ y transiente si $d\geq3$}.
...
Nos tornamos ahora a una pregunta que ya hemos tocado varias veces: ¿debiéramos esperar que, si \hlg{existe una distribuci{\'o}n invariante $\pi$}, entonces \hlo{la distribuci{\'o}n de la cadena converja a $\pi$}?
...
\begin{thm}\tbf{Convergencia al equilibrio}\label{thm:equil}\MarginNote{Convergencia al equilibrio}
Sea \hlg{$X$ una cadena de Markov irreducible y aperi{\'o}dica con distribuci{\'o}n invariante $\pi$}.
Entonces \hlo{para cualquier condici{\'o}n inicial $\mu$ se tiene}
...
Para demostrar esto usaremos una técnica muy importante en la teoría de probabilidades, conocida como\MarginNote{Coupling} \emph{\hlb{coupling}}.
El ingrediente base es la construcción dada en el siguiente resultado:

\begin{prop}\label{prop:couplingMC}\MarginNote{Coupling de una CM}
Sean \hlg{$X\sim\CM(\mu,P)$ e $Y\sim\CM(\nu,Q)$ cadenas de Markov irreducibles, aperi{\'o}dicas e independientes entre s{\'i}}, definidas en espacios de estados $I_1$ e $I_2$.
Entonces \hlo{el proceso $(X,Y)=((X_n,Y_n))_{n\geq0}$ a valores en $I_1\times I_2$ es $\CM(\mu\otimes\nu,P\otimes Q)$} donde $\mu\otimes\nu$ es la medida producto y
\[(P\otimes Q)_{(i,k)(j,\ell)}=p_{ij}q_{k\ell}.\]
Más aún \hlo{$(X,Y)$ es irreducible y aperi{\'o}dica}, y si además $X$ e $Y$ son recurrentes positivas con distribuciones invariantes $\pi_X$ y $\pi_Y$ entonces $(X,Y)$ es recurrente positiva con distribución invariante $\pi_X\otimes\pi_Y$.
\end{prop}

\begin{rem}
La cadena $(X,Y)$ construida en la proposición \hly{es lo que se conoce como un coupling}, o acoplamiento, de las dos cadenas.
La idea es que \hly{las marginales de la cadena acoplada son justamente $X$ e $Y$}.
Este es un coupling particular, donde \hly{las dos cadenas evolucionan de manera independiente}; en otros contextos se utilizan construcciones más complicadas, pero para nuestros propósitos esta construcción será suficiente.
\end{rem}
...
\begin{prop}\MarginNote{Descomposición del espacio $I$}
Sea \hlg{$X$ irreducible, con periodo $d$}.
Entonces \hlo{existe una partición $C_0,\dotsc,C_{d-1}$ de $I$ tal que para cada $r\in\{0,\dotsc,d-1\}$ se tiene}:
\begin{enumerate}[label=\uptext{(\arabic*)}]
...
\begin{thm}\tbf{Convergencia al equilibrio, caso general}\label{thm:eq-gen}\MarginNote{Convergencia al equilibrio, caso general}
Sea \hlg{$X$ irreducible, con periodo $d$, y sea $C_0,\dotsc,C_{d-1}$ la partición dada en la proposición anterior}.
Supongamos que \hlg{$X_0\in C_0$ con probabilidad $1$}.
\hlo{Entonces}
\[\lim_{n\to\infty}\pp(X_{nd+r}=j)=\frac{d}{\ee_j(\tau_j)}\qquad\forall\,j\in C_r.\]
\end{thm}

\begin{rem}
El teorema contiene información nueva incluso en el caso aperiódico, pues nos dice que \hlg{cuando $X$ no es recurrente positivo} se tiene \hlo{$\lim_{n\to\infty}\pp(X_{n}=j)=0$ para todo $j$}.
